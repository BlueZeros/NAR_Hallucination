...
from data import S2SDataset, WizardOfWikipediaDataset
...
from eval import Evaluate, eval_acc
...
def train(config):
    ...
    train_dataloader = DataLoader(
        WizardOfWikipediaDataset(data_dir=config["data_dir"], tokenizer=tokenizer, data_usage="train"),
    ...
    valid_dataloader = DataLoader(
        WizardOfWikipediaDataset(data_dir=config["data_dir"], tokenizer=tokenizer, data_usage="valid"),
    ...
    for epoch_idx in range(config["start_epoch"], config["epochs"]):
        ...
        with torch.no_grad():
            ...
            cache = eval_acc(None, json.load(open(os.path.join(config["data_dir"], "valid_seen.json"))), compute_cache=True)
            ...
            metric_dict.update(eval_acc(generated_text, None, cache=cache))
            logger.info("\n\nEpoch {} time {}: validation loss {} "
                        "BLEU-1: {} ROUGE_L: {} METEOR: {} KF1: {} EntF1: {} ACC: {}.\n".format(epoch_idx, valid_time, valid_loss,
                                                                             metric_dict["Bleu_1"],
                                                                             metric_dict["ROUGE_L"],
                                                                             metric_dict["METEOR"],
                                                                             metric_dict["KF1"],
...
def test(config):
    ...
    test_dataloader = DataLoader(
        WizardOfWikipediaDataset(data_dir=config["data_dir"], tokenizer=tokenizer, data_usage="test"),
    ...
    reference_text = []
    with torch.no_grad():
        cache = eval_acc(None, json.load(open(os.path.join(config["data_dir"], "test_seen.json"))), compute_cache=True)
