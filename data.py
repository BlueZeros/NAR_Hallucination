...
class WizardOfWikipediaDataset(Dataset):
    ...
    def prepare_data(self):
        ...
        try:
            ...
            with open(os.path.join(self.data_dir, '{}.json'.format(self.data_usage)), 'r') as f:
                file = json.load(f)
            ...
            for line in file:
                ...
                for i in line['dialog']:
                    utt = i['text']
                    if tmp_source != '':
                        source_data.append(tmp_source)
                        label_data.append("__start__ " + utt + " __end__")
            ...
            for idx in range(len(source_data)):
                src_ids = self.tokenizer.encode(source_data[idx], add_special_tokens=True, truncation=True, max_length=512)
                lb_ids = self.tokenizer.encode(label_data[idx], truncation=True, max_length=512)
                source_input_ids.append(src_ids)
                labels.append(lb_ids)
